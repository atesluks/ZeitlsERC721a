...
const TOKEN_NAME = 'ZEITS_TEST';
const TOKEN_SYMBOL = 'ZTS_T';
const TOKEN_URI = 'http://example.com/';
      ...
      context('with minted tokens', async function () {
        ...
        describe('ERC721Metadata support', async function () {
          it('responds with the right name', async function () {
            expect(await this.erc721a.name()).to.eq(TOKEN_NAME);
          })

          it('responds with the right symbol', async function () {
            expect(await this.erc721a.symbol()).to.eq(TOKEN_SYMBOL);
          })

          describe('tokenURI', async function () {
            it('sends a valid uri', async function () {
              let tokenId = 1;
              let uri = await this.erc721a['tokenURI(uint256)'](tokenId);
              expect(uri).to.eq(TOKEN_URI + tokenId + '.json');

              tokenId = 3;
              uri = await this.erc721a['tokenURI(uint256)'](tokenId);
              expect(uri).to.eq(TOKEN_URI + tokenId + '.json');
            })
...
describe('ZeitsNFT', createTestSuite({ contract: 'ZeitsNFT', constructorArgs: [TOKEN_NAME, TOKEN_SYMBOL, TOKEN_URI] }));
